...
check_ci_status() {
  ...
  while true; do
    ...
    # If there are failed checks, wait
      ...
      if [ "$ALLOW_AUTO_MERGE" == true ] && [ "$successful_count" -ge 2 ]; then
        echo "Two checks are successful. Merging the PR automatically..."
        merge_pr
      else
        echo "Conditions for auto-merge are not met."
      fi
      break
    fi
