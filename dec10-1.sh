...
check_ci_status() {
  ...
  while true; do
    ...
    # If all checks are completed and there are no failed checks
    elif [ $successful_count -eq $total_checks ] && [ $total_checks -ge 2 ]; then
      echo "All checks have passed and there are at least 2 successful checks."
      ...
      echo "Not all checks have passed or there are fewer than 2 successful checks. Waiting..."
    fi
